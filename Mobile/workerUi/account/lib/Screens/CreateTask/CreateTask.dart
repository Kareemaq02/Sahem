...
class _CreateTaskState extends State<CreateTask> {
  ...
  List<ComplaintModel> lstComplaints = [];
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => AddComplaintPage(
          lstComplaints: lstComplaints,
          onAddComplaint: (complaint) {
            lstComplaints.add(complaint);
          },
        ),
      ),
    );
  ...
  void _sendRequest() async {
    final response = await CreateTaskRequest().insertTask(
      ...
      lstComplaints.map((e) => e.intComplaintId).toList(),
    );
        ...
        child: Column(
          ...
          children: [
            ...
            ComplaintsAdd(
              ...
              onComplaintRemove: (complaint) => setState(
                () => lstComplaints.remove(complaint),
              ),
            ),
            // The three fields
            Container(
            ...
            Padding(
              ...
              child: StyledButton(
                ...
                fontSize: 16,
                onPressed: () {
                  if (lstComplaints.isEmpty) {
                    _showSnackbar(context, "لم يتم اضافة بلاغ لهذا العمل");
                    return;
                  }
